...
export function getMyTeams(signal?: AbortSignal) {
  return apiClient.get<{ team: Team; role: string }[]>("/api/teams/mine", signal);
}
...
export function getAvailableRepositories(signal?: AbortSignal) {
  return apiClient.get<AvailableRepository[]>("/api/repositories/available", signal);
}

export interface GitGraphCommitResponse {
  hash: string;
  parents: string[];
  author: string;
  authorEmail: string;
...
export interface GitGraphBranchResponse {
  name: string;
  head: string;
  color: string;
  isRemote: boolean;
...
export function getGitGraph(repositoryId: number, signal?: AbortSignal) {
  return apiClient.get<{ commits: GitGraphCommitResponse[]; branches: GitGraphBranchResponse[] }>(
    `/api/repositories/${repositoryId}/git-graph`,
    signal
  );
}
